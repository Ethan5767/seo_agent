...
|---|---|---|---|
| 0 | ~~RUN the outstanding migrations~~ **DONE 2026-09-13.** `web/migrations/RUN-THIS.sql` applied via the Supabase SQL editor | Verified from outside with the public anon key straight afterwards: `traffic_snapshots` `0-2/170` -> `*/0`, orphans gone, all ten per-user tables `*/0`, `remediations` now exists. `npm run db:check` prints **In sync. No migrations outstanding.** Re-run it after any schema change | ☑ |
| 0b | ~~RUN `web/migrations/2026-09-13-b091-traffic-snapshots-rls.sql` against the live Supabase database.** Until it runs, `traffic_snapshots` is readable by anyone holding the public anon key | B-091. The policy was `for all using (true) with check (true)` on a table with a `user_id` column, so RLS was on and enforcing nothing. Measured 2026-09-13: an anonymous read returned **170 of 170 rows** with site_url, clicks, impressions, CTR, average position, and the real `top_queries` a customer's visitors typed. The migration deletes the 170 unattributable rows (all have `user_id` NULL; the table is a regenerable cache), makes the column `not null`, and replaces the policy. **A schema fix in the repo changes nothing until it is applied** | ☐ |
| 1 | Thin callers copied from `.github/examples/`, pinned to an exact tag (`@v3.2.0`, never `@main`, never `@v3`) | A mutable ref means the thing guarding production can change without a PR. `tests/test_pipeline_pin.py` enforces this here. **Every client on `@v3.1.3` or `@v3.1.4` must be bumped to `@v3.2.0`** — both of those carry B-063, which fails the gate on every pull request regardless of the code under review | ☐ |
...
| 6 | Branch protection on the **client's** default branch | **Cannot be enabled on GitHub Free for a private repo.** The gate reports but cannot block. Needs a paid plan, or a public client repo. (`seo_agent` itself is public since 2026-08-11 and therefore *could* have protection; as of 2026-09-13 it has none — `gh api repos/Ethan5767/seo_agent/branches/main/protection` -> 404, `.../rulesets` -> `[]`) | ☐ |
