...
**As of 2026-09-13 (v3.2.0)** · 9 packages, 83 modules, 5 workflows, 41 `wf-*` commands, **1,257 Python tests (all pass) + 352 web tests (all pass)**. Counted on 2026-09-13, not carried forward: `find pipeline -name '__init__.py' | wc -l`, `find pipeline -name '*.py' ! -name '__init__.py' | wc -l`, `ls .github/workflows/*.yml | wc -l`, the `wf-` entries under `[project.scripts]`, `pytest -q` and `npm test`. The line this replaced read "As of 2026-08-10 · 5 packages, 40 modules · 720 tests" — more than a month stale, and wrong about the package count by four. The six 2026-09-12 subsystems previously listed as UNVERIFIED are still unverified **against a live network**, which is a different claim from untested — see the CHANGELOG.
