...
|---|---|---|---|
| 1 | Thin callers copied from `.github/examples/`, pinned to an exact tag (`@v3.1.4`, never `@main`, never `@v3`) | A mutable ref means the thing guarding production can change without a PR. `tests/test_pipeline_pin.py` enforces this here | ☐ |
