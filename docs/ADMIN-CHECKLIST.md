...
|---|---|---|---|
| 0 | **RUN `web/migrations/2026-09-13-apply-outstanding.sql` in the Supabase SQL editor.** It supersedes the B-091 file below and covers everything outstanding | Measured 2026-09-13 with `npm run db:check`, not assumed: the live database is otherwise in sync. Exactly two things are missing - the `remediations` table, and the `traffic_snapshots` policy. **The migration DELETEs 170 rows** (all `user_id` NULL, a regenerable Search Console cache). Re-run `npm run db:check` afterwards; it must print "In sync." | ☐ |
...
| 1 | Thin callers copied from `.github/examples/`, pinned to an exact tag (`@v3.1.3`, never `@main`, never `@v3`) | A mutable ref means the thing guarding production can change without a PR. `tests/test_pipeline_pin.py` enforces this here | ☐ |
