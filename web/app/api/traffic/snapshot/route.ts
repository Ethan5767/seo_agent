...
export async function POST(req: NextRequest) {
  try {
    ...
    const snapshot = {
      ...
      clicks: Number.isFinite(Number(clicks)) ? Number(clicks) : null,
      impressions: Number.isFinite(Number(impressions)) ? Number(impressions) : null,
      ctr: Number.isFinite(Number(ctr)) ? Number(ctr) : null,
      avg_position: Number.isFinite(Number(avgPosition)) ? Number(avgPosition) : null,
