...
| ~~`pages-are-data-check.py`~~ | Pages come from 4 templates + data, not bespoke per-route `page.tsx`. Flags a static non-whitelisted route above `--max-static-lines` (default 120). Lesson-1 gate. | **REMOVED** | 1 | 1 | **GONE in `79b0b5b`** — emitter-bound, went with the DOCX rail. Row kept for the Run #1 record; the file does not exist. |
| ~~`brief-fanout-check.py`~~ | Each `docs/briefs/*.json` has ≥6 distinct fan-out queries, a capsule, ≥1 semantic triple, a `proprietary_variable` inside the allow-list, and a valid intent enum. §19. | **REMOVED** | 9 | 9 | **GONE in `79b0b5b`** — `docs/briefs/` was a `pipeline/generate` artifact and went with it. Row kept for the Run #1 record; the file does not exist. |
...
| ~~`validate_multistate_config.py`~~ | Multi-state addendum applies only when `topology_class` actually needs it | **REMOVED** | 1 | 0 (bypassed) | **GONE in `79b0b5b`** — emitter-bound. Row kept for the Run #1 record; the file does not exist. Note `url_topology` is still read by `lib/common.py`; only the gate went. |
...
| Gate | What it checks | Blocking? | Exit (registry) | Exit (observed Run #1) | Status |
|---|---|---|---:|---:|---|
| `orphan-check.py` | Every sitemap URL has ≥1 inbound internal `<a href>` from anywhere in the built tree. **The original Acme bug — highest-value gate in the suite.** Scope worth knowing before relying on it: **self-links count** (a page reached by global nav links itself, deliberately — see the module docstring), and it iterates the *sitemap*, so a built route that never reached the sitemap is never evaluated (that one is `parity-check`'s). It catches a page in no nav/footer/body anywhere; it does not model hub→child completeness. | BLOCKING | 3 | 1 | **WORKS** — 99/99 clean; seeded orphan `<loc>` went red |
...
| `capsule-check.py` | Interrogative H2 (leads with a question word or ends in `?`) → the first `<p>` **or `<li>`** under it, up to the next heading, at **40–80 words and ≤3 sentences** → TL;DR on pages over `content.long_page_threshold`. Selects every route fitting the client's `topology` plus `/blog/*`, minus `DEFAULT_EXCLUDE`. §20 — the literal unit AI engines lift. | BLOCKING | 6 | 6 | **WORKS + REAL FINDING** — 60/61 Acme pages lack a capsule |
