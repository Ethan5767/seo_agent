...
def test_ranked_keywords_uses_injected_call():
    rows, status = ranked_keywords("x.com", call=lambda p, b: (DOC, None))
    assert status.startswith("ok:")
    assert len(rows) == 3


def test_call_error_degrades_to_status_not_crash():
    rows, status = ranked_keywords("x.com", call=lambda p, b: (None, "skipped: creds unset"))
