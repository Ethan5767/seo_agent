...
|---|---|---|---|
| 0 | ~~RUN the outstanding database migrations~~ **DONE 2026-09-13.** `web/migrations/RUN-THIS.sql` applied via the Supabase SQL editor | Closes B-091. Verified from outside with the PUBLIC anon key straight afterwards: `traffic_snapshots` went `0-2/170` -> `*/0`, the 170 orphan rows are gone, all ten per-user tables return `*/0`, and `remediations` exists for the first time. `npm run db:check` prints **In sync. No migrations outstanding.** Re-run it after any schema change | ☑ |
| 1 | Thin callers copied from `.github/examples/`, pinned to an exact tag (`@v3.2.0`, never `@main`, never `@v3`) | A mutable ref means the thing guarding production can change without a PR. `tests/test_pipeline_pin.py` enforces this here. **Every client on `@v3.1.3` or `@v3.1.4` must be bumped to `@v3.2.0`** — both of those carry B-063, which fails the gate on every pull request regardless of the code under review, while the sticky comment says GREEN | ☐ |
