...
| `dataforseo.py` | The DataForSEO client + one pure parser per tool: `cost_of`/`result_items`/`_tool` primitives; `site_audit` (on-page crawl → Site Health), `ranked_keywords`/`domain_overview`/`serp_rank`/`rankings` (Rankings), `search_volume`/`keyword_ideas`/`keyword_gap`/`competitors`/`keywords_card` (Keywords), `llm_mentions` (AI Visibility), `backlink_gap` + `bare_domain` (Backlink Gap, backlinks/domain_intersection), `compare_domains` (Compare Domains: your overview next to up to 3 named or discovered competitors'), `backlink_overview` (Backlink Overview: summary link profile, top referring domains and anchors, monthly history; shapes verified on sandbox.dataforseo.com). Parsers unit-tested offline with canned responses; exact cost from each response's `cost` field. |
...
| `eeat.py` | The E-E-A-T checker: 17 signals across Experience, Expertise, Authoritativeness and Trust, read from JSON-LD (`jsonld_objects` walks `@graph` and nesting) and visible text, each row carrying its evidence (author name and title, cited domain, accreditation sentence, rating and count). Health pages (YMYL) get the stricter severity and two extra checks (medically reviewed, medical disclaimer). No total score: Google has none. `eeat_rows(html, url)`. |
